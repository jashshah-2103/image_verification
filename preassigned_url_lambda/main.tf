...
resource "aws_apigatewayv2_route" "s3upload" {
  ...
  route_key      = "POST /${aws_lambda_function.preassnurl.function_name}"
...
resource "aws_lambda_permission" "apigw-permission" {
  ...
  function_name = aws_lambda_function.preassnurl.arn
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.s3upload.execution_arn}/*/*/${aws_lambda_function.preassnurl.function_name}"
}
